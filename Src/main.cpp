...
mvStack.top() = glm::rotate(mvStack.top(), float(180.0f * DEG_TO_RADIAN), glm::vec3(10.0f, 0.0f, 0.0f));
...
//bunnyDraw(tmp, projection, offset);
...
uniformIndex = glGetUniformLocation(reflectProgram, "ratioR");
	...
	rt3d::setLightPos(reflectProgram, glm::value_ptr(tmp));
	glBindTexture(GL_TEXTURE_2D, textures[2]);
	mvStack.push(mvStack.top());
...
mvStack.pop();
	...
	rt3d::setUniformMatrix4fv(reflectProgram, "modelview", glm::value_ptr(mvStack.top()));
	rt3d::setMaterial(reflectProgram, currentMaterial);
