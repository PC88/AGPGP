...
void MonoChromDemo::Update(double interval)
{
	...
	if (keys[SDL_SCANCODE_W]) eye = moveForward(eye, r, 0.51f);
	if (keys[SDL_SCANCODE_S]) eye = moveForward(eye, r, -0.51f);
	if (keys[SDL_SCANCODE_A]) eye = moveRight(eye, r, -0.51f);
	if (keys[SDL_SCANCODE_D]) eye = moveRight(eye, r, 0.51f);
...
void MonoChromDemo::Render()
{
	...
	int uniformIndex =
		glGetUniformLocation(shaderProgram_phong_monochrome,
			"attConst");
...
glm::vec3 MonoChromDemo::moveForward(glm::vec3 pos, GLfloat angle, GLfloat d)
{
